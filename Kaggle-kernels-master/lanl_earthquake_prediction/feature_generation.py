...
OUTPUT_DIR = r'./output'  # set for local environment
DATA_DIR = r'.'  # set for local environment

SIG_LEN = 150000
NUM_SEG_PER_PROC = 1000
NUM_THREADS = 24
...
def split_raw_data():
    df = pd.read_csv(os.path.join(DATA_DIR, 'train.csv'),dtype={'acoustic_data': np.int16, 'time_to_failure': np.float32})
    ...
    for i in range(NUM_THREADS):
        ...
        df0.to_csv(os.path.join(OUTPUT_DIR, 'raw_data_%d.csv' % i), index=False)
        del df0
...
def build_rnd_idxs():
    ...
    max_start_idx = 26214390 #int(629145480/NUM_THREADS)
...
def build_fields(proc_id):
    ...
    try:
        ...
        train_df = pd.read_csv(os.path.join(OUTPUT_DIR, 'raw_data_%d.csv' % proc_id), dtype={'acoustic_data': np.int16, 'time_to_failure': np.float32})
